...
const LightChaserList = () => {
  ...
  useEffect(() => {
    ...
    const {
      projectConfig: { saveType = SaveType.LOCAL },
    } = designerStore;
    abstractOperatorMap[saveType]
      .getProjectSimpleInfoList()
      .then((data: any) => {
        if (data && data.length > 0) {
          ...
          data.forEach((item: any) => {
            ...
            if (imageId && imageId !== "") imageIds.push(imageId);
          });
          const promise = imageIds.map((imageId: any) =>
            ImgUtil.getImageFromLocalWithKey(imageId)
          );
  ...
  const onCancel = () => setShowAddDialog(false);

  const openScreen = (e: any) => {
    ...
    if (type === "edit") {
      ...
      abstractOperatorMap[saveType].deleteProject(id);
    }
  };
        ...
        <div className={"console-list"}>
          <div className={"console-list-item"}>
            <div className={"item-icon"}>
              <img src={listBottom} alt={"项目列表"} />
            </div>
            <div className={"item-text"}>项目列表</div>
          </div>
          <div className={"console-list-item"}>
            <div className={"item-icon"}>
              <img src={datasource} alt={"数据源管理"} />
            </div>
            <div className={"item-text"}>数据源管理</div>
          </div>
          <div className={"console-list-item"}>
            <div className={"item-icon"}>
              <img src={templateMarket} alt={"模板市场"} />
            </div>
        ...
        <div className={"console-content"}>
          <div className={"content-body"}>
            <div className={"project-list"}>
              ...
              >
                <LcButton
                  onClick={toggleNewProVisible}
                  style={{ width: width, height: height, fontSize: 20 }}
                >
                  + 新建项目
                </LcButton>
              </div>
