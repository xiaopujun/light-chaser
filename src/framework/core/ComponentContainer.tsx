...
export interface ComponentContainerProps {
    ...
    triggerSource?: string; //渲染源，可能来自编辑模式，预览模式，或大屏引用组件
}

const ComponentContainer = memo((props: ComponentContainerProps) => {
    const {layer, layerManager, bpExecutor, triggerSource} = props;
    ...
    useEffect(() => {
        ...
        if (componentDefine) {
            ...
            if (Controller) {
                ...
                controller.create(ref.current!, config, bpExecutor).then(() => {
    ...
    return (
        <Suspense fallback={<Loading/>}>
            <div
                ...
                }} className={`lc-comp-item${triggerSource || ''}`}>
                <div ref={ref} style={{
                    width: '100%',
                    height: '100%',
...
const registerProxy = (compId: string, controller: AbstractDesignerController) => {
    controller.create = new Proxy(controller.create, {
        apply(target, thisArg, argus) {
            ...
            controller.bpExecutor?.triggerComponentEvent(compId, "loaded", controller.config);
            return proxy;
        }
    });
    controller.changeData = new Proxy(controller.changeData, {
        apply(target, thisArg, argus) {
            controller.bpExecutor?.triggerComponentEvent(compId, "dataChange", argus[0]);
